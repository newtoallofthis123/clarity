...
function isLiked(user?: FullUser, post_id?: string) {
  let liked = false;
  user?.likes.map((post) => {
    if (post.post_id === post_id) {
      liked = true;
...
export default function LikeComponent({ user, post }: Props) {
  ...
  function handleLike(user?: FullUser, post_id?: string) {
  ...
  return (
    <div>
      <button
        onClick={() => {
          handleLike(user, post?.id);
        }}
      >
        {isLiked(user, post?.id) ? (
          <i className="bi bi-caret-up-fill text-2xl text-green-500 pr-2"></i>
        ) : (
          <i className="bi bi-caret-up text-2xl pr-2"></i>
        )}
      </button>
      {post?.likes.length}
    </div>
